...
async def citizen_feedback(callback: CallbackQuery, bot: Bot) -> None:
    ...
    # اگر شهروند گفته مشکل برطرف نشده، به گروه ادمین اطلاع بده تا دوباره بررسی شود
    if answer == "no" and report["admin_chat_id"] and report["admin_message_id"]:
        try:
            await bot.send_message(
                chat_id=report["admin_chat_id"],
                text=(
                    f"⚠️ بازخورد منفی از شهروند — {db.report_code(report_id)}\n"
                    "شهروند اعلام کرده مشکل هنوز برطرف نشده. لطفاً دوباره بررسی شود."
                ),
