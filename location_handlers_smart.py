...
import location_intelligence as intelligence
...
async def receive_location(
    ...
    candidates = intelligence.find_duplicate_candidates(
        latitude=latitude,
        longitude=longitude,
    ...
    if candidates:
        ...
        confidence = intelligence.classify_duplicate_distance(distance)
        confidence_text = {
            "high": "🟢 اطمینان بالا",
        ...
        repeat_count = int(nearby.get("report_count", 0))

        extra = ""
        if len(candidates) > 1:
            extra = f"\n🔎 تعداد نقاط مشابه نزدیک: <b>{len(candidates)}</b>"

        await message.answer(
            f"🔎 <b>احتمال گزارش تکراری پیدا شد</b>\n\n"
            f"کد پرونده: <code>{nearby['code']}</code>\n"
            f"نوع مشکل: {issue_name}\n"
            f"فاصله تقریبی: {distance} متر\n"
            f"وضعیت تشخیص: <b>{confidence_text}</b>\n"
            f"گزارش‌های قبلی این نقطه: <b>{repeat_count}</b>"
            f"{extra}\n\n"
