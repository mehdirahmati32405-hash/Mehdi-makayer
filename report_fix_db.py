...
def init_db() -> None:
    with get_conn() as conn:
        ...
        if "submission_key" not in existing_columns:
            conn.execute("ALTER TABLE reports ADD COLUMN submission_key TEXT")
        # کلید یکتای ثبت نهایی، جلوی ایجاد دو رکورد برای یک پیام تأیید را می‌گیرد.
        conn.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS idx_reports_submission_key "
            "ON reports(submission_key) WHERE submission_key IS NOT NULL"
        )
...
def create_report(
    ...
    """Create a report exactly once for a given submission_key.

    Returns (report_id, created). The unique database index makes this safe
    even when two callback handlers/processes reach the database together.
    """
    with get_conn() as conn:
        if submission_key:
            cursor = conn.execute(
                ...
                INSERT OR IGNORE INTO reports
                    (user_id, user_name, category, description, latitude, longitude,
                     photo_file_id, extra_photos, voice_file_id, is_anonymous,
                     submission_key, status, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    user_id,
            ...
            if cursor.rowcount == 0:
                row = conn.execute(
                    "SELECT id FROM reports WHERE submission_key = ?",
                    (submission_key,),
                ).fetchone()
                if row is not None:
                    return int(row["id"]), False
                raise RuntimeError("ثبت گزارش با کلید یکتا ناموفق بود.")
...
def reopen_report(report_id: int) -> None:
    """گزارشی که قبلاً «رسیدگی شد» بوده را دوباره به وضعیت اولیه برمی‌گرداند
    و بازخورد قبلی را پاک می‌کند تا سابقه تازه‌ای از رسیدگی مجدد ثبت شود."""
    with get_conn() as conn:
        conn.execute(
            "UPDATE reports SET status = ?, feedback = NULL WHERE id = ?",
            (STATUS_RECEIVED, report_id),
        )
...
# ---------------------------------------------------------------------------
# اشتراک اطلاع‌رسانی عمومی
# ---------------------------------------------------------------------------

def subscribe_user(user_id: int, user_name: str) -> None:
    with get_conn() as conn:
        conn.execute(
            "INSERT INTO subscribers (user_id, user_name, subscribed_at) VALUES (?, ?, ?) "
...
def get_all_subscribers() -> list[sqlite3.Row]:
    with get_conn() as conn:
        cursor = conn.execute("SELECT * FROM subscribers ORDER BY id")
        return cursor.fetchall()
...
# ---------------------------------------------------------------------------
# تقدیر و تشکر از عوامل خدمات شهری
# ---------------------------------------------------------------------------

def save_appreciation(user_id: int, user_name: str, text: str) -> int:
    with get_conn() as conn:
        cursor = conn.execute(
            "INSERT INTO appreciations (user_id, user_name, text, created_at) VALUES (?, ?, ?, ?)",
            (user_id, user_name, text, datetime.now(timezone.utc).isoformat()),
        )
        return cursor.lastrowid
