...
async def submit_report(callback: CallbackQuery, state: FSMContext, bot: Bot, is_anonymous: bool) -> None:
    ...
    # شناسه یکتای همین پیام تأیید است؛ حتی اگر callback دوبار/همزمان برسد،
    # دیتابیس فقط یک گزارش ایجاد می‌کند. برای yes و anonymous نیز عمداً یک کلید داریم.
    if callback.message is not None:
        submission_key = f"confirm:{callback.message.chat.id}:{callback.message.message_id}"
    else:
        # این مسیر عملاً نباید رخ دهد، ولی برای ایمنی کلید موقت می‌سازیم.
        submission_key = None

    report_id, created = db.create_report(
        user_id=user.id,
    ...
    if not created:
        await state.clear()
        await callback.answer("این گزارش قبلاً ثبت شده است. گزارش دیگری ایجاد نشد. ✅", show_alert=True)
        return
